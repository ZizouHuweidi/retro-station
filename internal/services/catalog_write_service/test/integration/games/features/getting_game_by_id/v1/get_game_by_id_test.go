...
func TestGetGameById(t *testing.T) {
	...
	RunSpecs(t, "Get Game By ID Integration Tests")
}
...
var _ = Describe("Get Game by ID Feature", func() {
	var (
		ctx    context.Context
		id     uuid.UUID
		query  *getGameByIdQuery.GetGameById
		result *dtos.GetGameByIdResponseDto
		err    error
	)
	...
	// "Scenario" step for testing returning an existing game with correct properties
	Describe("Returning an existing game with valid Id from the database with correct properties", func() {
		Context("Given games exists in the database", func() {
			BeforeEach(func() {
				query, err = getGameByIdQuery.NewGetGameById(id)
			})

			// "When" step
			When("the GteGameById query is executed for existing game", func() {
				BeforeEach(func() {
					result, err = mediatr.Send[*getGameByIdQuery.GetGameById, *dtos.GetGameByIdResponseDto](
						ctx,
						query,
					)
				})
				...
				It("Should return a game with the correct ID", func() {
					Expect(result.Game).NotTo(BeNil())
					Expect(result.Game.GameId).To(Equal(id))
				})
			})
		})
	})
	...
	Describe("Returning a NotFound error when game with specific id does not exist", func() {
		Context("Given games does not exists in the database", func() {
			BeforeEach(func() {
			...
			// "When" step
			When("the GteGameById query is executed for non-existing game", func() {
				BeforeEach(func() {
					result, err = mediatr.Send[*getGameByIdQuery.GetGameById, *dtos.GetGameByIdResponseDto](
						ctx,
						query,
				...
				It("Should return a NotFound error", func() {
					Expect(err).To(MatchError(ContainSubstring("error in getting game with id")))
				})
